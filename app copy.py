...
import pandas as pd
import time
from datetime import datetime, date, timedelta  # --- PERBAIKAN DI SINI ---
import uuid
...
def load_tasks_from_db():
    ...
    for row in tasks_rows:
        task = dict(row)
        for date_col in ["Tanggal Mulai", "Tanggal Selesai Target", "Tanggal Jadwal", "Tanggal Selesai"]:
...
st.title("Personal Productivity Hub")
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Dashboard & Input", "Prioritaskan", "Jadwal Aktivitas", "Kalender", "Sesi Fokus", "Delegasikan"])
...
with tab2:
    ...
    if not tasks_aktif: st.info("Tidak ada tugas aktif untuk diprioritaskan.")
    else:
        df_aktif = pd.DataFrame(tasks_aktif)
        st.dataframe(df_aktif[['Tugas', 'Prioritas', 'Tanggal Mulai', 'Tanggal Selesai Target', 'Deskripsi']], use_container_width=True, hide_index=True)
    st.markdown("---")
    ...
    else: st.success("Semua tugas aktif sudah diprioritaskan!")
    st.markdown("---")
    ...
    if tasks_needing_schedule:
        ...
        with st.form("set_schedule_form"):
            for task in tasks_needing_schedule: st.date_input(f"Tanggal eksekusi untuk '{task.get('Tugas')}'", min_value=date.today(), key=f"date_in_{task['id']}")
