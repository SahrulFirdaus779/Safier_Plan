...
# --- FUNGSI-FUNGSI DATABASE (DENGAN PERBAIKAN) ---
DB_FILE = "tasks.db"
...
def setup_database():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS tasks (
            ...
            "Tanggal Jadwal" TEXT, "Tanggal Selesai" TEXT
        )
    """)
    conn.commit()
    conn.close()
...
def get_task_status(task):
    if task.get('Selesai'): return "Completed", "#28a745"
    if task.get('Prioritas') == 'Lakukan Sekarang': return "In Progress", "#fd7e14"
    if task.get('Prioritas') == 'Jadwalkan': return "Scheduled", "#17a2b8"
    return "Not Started", "#6c757d"
...
with tab6:
    ...
    if not tasks_to_delegate: st.info("Tidak ada tugas untuk didelegasikan.")
    else:
        st.warning("Tugas berikut telah diidentifikasi untuk didelegasikan.")
        with st.form("delegation_form"):
            delegation_inputs = {}
            for task in tasks_to_delegate: delegation_inputs[task['id']] = st.text_input(f"Delegasikan '{task.get('Tugas')}' kepada:", value=task.get('Delegasi', ''), key=f"delegate_{task['id']}")
            if st.form_submit_button("Simpan Informasi Delegasi"):
                for task_id, delegated_to in delegation_inputs.items():
                    task = get_task_by_id(task_id)
                    if task: 
                        task['Delegasi'] = delegated_to
                        save_task_to_db(task)
                st.success("Informasi delegasi berhasil disimpan!"); st.rerun()

# --- (BARU) TAB 7: LAPORAN ---
with tab7:
    st.header("Laporan & Analisis Produktivitas")
    ...
    if not st.session_state.tasks:
        ...
        # --- GRAFIK 1: TUGAS SELESAI 7 HARI TERAKHIR ---
        st.subheader("Tugas Selesai dalam 7 Hari Terakhir")
        
        # --- PERBAIKAN FINAL DI SINI ---
        # Hanya proses jika kolom 'Tanggal Selesai' ada dan tidak kosong
        if 'Tanggal Selesai' in df_all_tasks.columns and not df_all_tasks['Tanggal Selesai'].isnull().all():
            # Konversi kolom 'Tanggal Selesai' dan pastikan tipe datanya adalah date
            ...
            completed_tasks_recent = df_all_tasks[
                (df_all_tasks['Selesai'] == True) &
                (df_all_tasks['Tanggal Selesai'] >= seven_days_ago) &
                (df_all_tasks['Tanggal Selesai'] <= today)
            ]
            ...
            if completed_tasks_recent.empty:
                ...
                fig_bar = px.bar(
                    merged_data, x='Tanggal Selesai', y='Jumlah',
                    title='Jumlah Tugas Selesai per Hari',
                    labels={'Tanggal Selesai': 'Tanggal', 'Jumlah': 'Jumlah Tugas'}, text_auto=True
                )
        ...
        # --- GRAFIK 2: DISTRIBUSI PRIORITAS TUGAS AKTIF ---
        st.subheader("Distribusi Prioritas Tugas Aktif")
        ...
        if active_tasks_df.empty:
            ...
            fig_pie = px.pie(
                priority_counts, names='Prioritas', values='Jumlah',
                title='Komposisi Prioritas Tugas yang Sedang Berjalan', hole=0.3
            )
